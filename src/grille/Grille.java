...
public class Grille {
	...
	public void insertPion(int y, int idJoueur) throws OutOfGrid { // gerer le
																	// cas ou on
																	// sort de
																	// la
																	// grille,
		...
		try {
			...
			for (i = 0; i < this.x; i++) {
				if (this.grille[i][y].getIdJoueur() != 0) { // si on on trouve
															// un autre pion
	...
	public void deletePion(int y) throws OutOfGrid { // gerer le cas ou on sort
														// de la grille, et plus
														// rarement pas bon id
		try {

			for (int i = 0; i < this.x; i++) {
				if (this.grille[i][y].getIdJoueur() != 0) { // si on on ntrouve
															// un autre pion
	...
	public boolean checkGridFull() {
		for (int i = 0; i < x; i++) {
			for (int j = 0; j < y; j++) {
				if (this.grille[i][j].getIdJoueur() == 0) { // si on a une case
															// vide
					return false;
				}
			}
	...
	private int detectWinCol(int x, int y) { // colone
		...
		if (id != 0) {
			for (int i = x + 1; i < this.x; i++) {// parcourt par colonne vers
													// le haut
			...
			for (int i = x - 1; i >= 0; i--) {// parcourt par colonne vers le
												// bas
				if (this.grille[i][y].getIdJoueur() != id) {
					break;
				} else if (this.grille[i][y].getIdJoueur() == id) {
	...
	private int detectWinLine(int x, int y) {// ligne
		...
		if (id != 0) {
			for (int i = y + 1; i < this.y; i++) {// parcourt par ligne vers la
													// droite
	...
	private int detectWinDiagLeft(int x, int y) {// Diagonal gauche
		...
		int j = y - 1;
		if (id != 0) {
			while (i >= 0 && j >= 0) { // parcourt diagonal vers en haut à
										// gauche
			...
			i = x + 1;
			j = y + 1;
			while (i < this.x && j < this.y) { // parcourt diagonal vers en bas
												// à droite
				if (this.grille[i][j].getIdJoueur() != id) {
					break;
				} else if (this.grille[i][j].getIdJoueur() == id) {
	...
	private int detectWinDiagRight(int x, int y) {// Diagonal droite
		...
		int j = y + 1;
		if (id != 0) {
			while (i >= 0 && j < this.y) { // parcourt diagonal vers en haut à
											// droite
				if (this.grille[i][j].getIdJoueur() != id) {
					break;
				} else if (this.grille[i][j].getIdJoueur() == id) {
			...
			i = x + 1;
			j = y - 1;
			while (i < this.x && j >= 0) { // parcourt diagonal vers en bas à
											// gauche
				if (this.grille[i][j].getIdJoueur() != id) {
					break;
				} else if (this.grille[i][j].getIdJoueur() == id) {
